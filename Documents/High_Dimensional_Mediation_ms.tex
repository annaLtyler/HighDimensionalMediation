...
complex traits is driven by weak trans-eQTLs. They proposed a framework
...
and explains a large proportion of disease risk.
...
the whole transcriptome with genes more functionally related to
metabolic disease receiving larger loadings. The strength of mediation
(transcript loading) was negatively correlated with local heritability
and positively correlated with distal heritability, suggesting that
distal gene regulation is the dominant mode through which gene
expression mediates the effect of genotype on phenotype. This pattern
also suggests that distal regulation may be quite strong for many
mediating genes. For example, \text{Nucb2}, was a strongly mediating
gene in islet and was also strongly distally regulated (66\% distal
...
individual transcripts or genetic loci in disease pathogenesis.
Historically, geneticists have been interested in pinpointing individual
loci whose mechanistic roles can be dissected through further
experiments. The continuous results, can also be quite informative,
however, and paired with increasing amounts of high-dimensional data in
public data bases can be useful for generating hypotheses and potential
drug treatments. We showed that ranked lists of genes can be analyzed
for enriched meaningfuil biological functions and pathways using GSEA.
These lists can be paired with information about cell-type specific
genes to generate hypotheses about cell composition in individual
tissues. Finally, we used the CMAP database to show that the
