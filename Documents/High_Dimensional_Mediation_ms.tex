...
\cite{pmid32912663} and tend not to co-localize with eQTL
\cite{pmid36515579, 
pmid37857933}.
...
Instead, what has been observed is that genes that are near GWAS hits
and have obvious functional relevance to a trait tend to have highly
...
core genes, as well as more peripheral genes that may not seem obviously
related to the trait.

Assessing the role of wide-spread distal gene regulation on complex
traits requires a large dedicated data set that incorporates
high-dimensional phenotyping, dense genotyping in a highly recombined
population, and transcriptome-wide measurements of gene expression in
multiple tissues to adequately assess whether missing heritability can
be explained through distal gene regulation or by identifying local gene
regulation in multiple relevant cell types.

distinguish the roles of distal gene regulation and

Both the univariate and omnigenic approaches discussed above posit
causal mediation models whereby variation in gene expression mediates
the effect of genotype on phenotype. In the case of the univariate
approaches, like TWAS and SMR, each transcript is tested independently
...
disease traits. In the univariate approach local and distal regulation
must be treated separately with huge numbers of statistical tests, which
is compuatationally expensive, requires strict corrections for multiple
testing, and incorrectly assumes independence of genetic variants and
transcripts. Such methods are therefore limited to detecting only the
largest statistical effects and are biased toward local gene regulation.
...
redundancy of the transcriptome without explicitly modeling them.
