...
Genotypes, phenotypes, and pancreatic islet gene expression data were
all previously published \cite{pmid29567659}, and are available through
Dryad (\url{doi:10.5061/dryad.pj105}; data files: Attie Islet eQTL data)
...
of trait filtering can be found on Synapse XXX in the file
1b.Trait\_Selection.Rmd.
...
objective function. Our starting point is the univariate case, where we
describe perfect mediation as a constraint on the covariance matrix
among variables. We then leverage this constraint to define projections
of multivariate data that are maximally consistent with perfect
mediation (HDMA). Next, we demonstrate how to \textit{kernelize} HDMA to
limit dimensionality of the model and enable non-linear HDMA models.
Finally, we discuss the connection between HDMA and related
\textit{multiblock} methods, including canonical correlation analysis
(CCA) and partial least squares path modeling (PLS-PM).
...
\textit{model-implied covariance 
matrix}, \(S\), given by
...
\({\rm tr}(\cdot)\) denotes the trace. For the perfect-mediation model,
these values are
...
\[F(\alpha,\beta) = -\log\left((1-\alpha^2)(1-\beta^2)\right) + \frac{2\alpha^2\beta^2}{(1-\alpha^2)(1-\beta^2)} + \frac{2\beta}{1-\beta^2} - 1,\]
...
\(\alpha\) and \(\beta\) between \(x\) and \(m\) and \(m\) and \(y\),
respectively.
...
\textit{sum-of-correlation}, or SUMCOR, optimization problems {[}REF{]},
...
converges at least to a stationary point. The overall idea is to use
...
\(F(\alpha, \beta)\) is irrelevant, so our objective function reduces to
...
(weighted) SUMCOR optimization problem {[}REF{]}. These optimization
...
in the span of the measured individuals, namely
...
among individuals, \(C_X = XX^T\), and reduces the dimensionality from
the number of measured variables down to the number of individuals
...
other linear models, including CCA {[}REF{]}, linear regression
{[}REF{]}, and classification {[}REF{]}.
...
non-linear functions of the measured variables. Promoting a linear model
to a non-linear model in this way is called the \textit{kernel trick}
and is widely used in the machine learning field. The above
considerations show that HDMA is kernelizable in the same way as other
linear models, although the exploration of non-linear models is outside
...
correlation coefficient \textit{per se} is numerically unstable due to
the inversion of the covariance matrices of the measured variables
(\textit{e.g.}, the transcript-transcript covariance matrix). To
overcome this, the RGCCA package uses a regularized form of the
covariance matrix developed by Schaeffer and Strimmer {[}REF{]}, which
can be estimated rapidly using an analytic formula.
