...
very low (0.14), suggesting that the these locally derived transcripts
...
HDL - high-density lipoprotein, TG - triglyceride. Source data are provided 
as Source Data files Fig1A-E.txt, Fig1F.txt, and Fig1G.txt.
}
...
high distal heritability. Source data are provided 
as Source Data files Fig2A.txt, Fig2B.txt, and Fig2C.txt}
...
Source data are provided as Source Data files Fig3B\_null.txt,
Fig3B\_stats.txt, Fig3C\_null.txt and Fig3C\_stats.txt.
}
\label{fig:workflow}
\end{figure}
...
Source data are provided as Source Data files Fig4A.txt, Fig4B.txt, Fig4C.txt,
Fig4D\_null.txt, and Fig4D\_observed.txt.
}
...
showing the loadings of transcripts selected by different criteria. Bar 
color indicates the tissue of origin. The heat map shows the local 
(L - left) and distal (D - right) heritability of each transcript. 
\textbf{A.} Loadings for the 10 transcripts with the largest positive 
loadings and the 10 transcripts with the largest negative loadings for 
each tissue. Distal heritability was significantly higher than local 
heritability (t-test $p < 2.2^{-16}$). \textbf{B.} Loadings of TWAS 
candidates with the 10 largest positive correlations with traits and 
the largest negative correlations with traits across all four tissues. 
Local and distal heritability were not significantly different for this 
group (t-test $p = 0.77$). \textbf{C.} The transcripts with the largest 
...
$p < 2.2^{-16}$, Source data are provided as Source Data files 
Fig5A.txt, Fig5B.txt, and Fig5C.txt.)
}
...
Source data are provided as Source Data files Fig6A.txt, Fig6B.txt,
and Fig6C.txt.
}
\label{fig:toa}
\end{figure}
...
Source data are provided as Source Data files Fig7B.txt
and Fig7C.txt.
}
...
patients based on the HDMA results from DO mice. Source data are provided 
as Source Data files Fig8A.txt, Fig8B.txt, Fig8C.txt, Fig8D1\_upper\_left.txt, 
Fig8D2\_upper\_right.txt, Fig8D3\_lower\_left.txt, Fig8D4\_lower\_right.txt.
}
