...
%DIF DEL Detailed_Methods_orig.tex   Sat Sep 21 06:38:01 2024
%DIF ADD Detailed_Methods_JMM.tex    Sat Sep 21 06:37:31 2024
...
\urlstyle{same}
\hypersetup{
  pdftitle={Online methods for: Transcripts with high distal heritability mediate genetic effects on complex traits},
  hidelinks,
...
\DIFdelbegin \DIFdel{Here }\DIFdelend \DIFaddbegin \DIFadd{In this section }\DIFaddend we derive the objective function for high-dimensional
...
perfect mediation (HDMA). \DIFdelbegin \DIFdel{Then}\DIFdelend \DIFaddbegin \DIFadd{Next}\DIFaddend , we demonstrate how to \textit{kernelize}
...
\textit{model-implied covariance matrix}, \DIFdelbegin \DIFdel{\(S\)}\DIFdelend \DIFaddbegin \DIFadd{\(\Sigma\)}\DIFaddend , given by

\begin{align}
\label{eqn:model_implied_covariance}
\DIFdelbegin \DIFdel{S }\DIFdelend \DIFaddbegin \DIFadd{\Sigma }\DIFaddend = 
  \begin{bmatrix}
1 & \alpha & \alpha \beta \\
\alpha & 1 & \beta \\
\alpha \beta & \beta & 1
...
%DIFDELCMD < %%%
\DIFdelend Note that the assumption of perfect mediation forces the
covariance between \(x\) and \(y\) to be \(\alpha \beta\). In any finite
data set, however, the observed covariance matrix, \DIFdelbegin \DIFdel{\(\Sigma = [\Sigma_{ij}]\)}\DIFdelend \DIFaddbegin \DIFadd{\(S = [S_{ij}]\)}\DIFaddend ,
will not typically satisfy this constraint.

The general \DIFaddbegin \DIFadd{negative }\DIFaddend log-likelihood \DIFaddbegin \DIFadd{fitting }\DIFaddend function for an SEM is given
by
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{bollen2014structural}
}\hskip0pt%DIFAUXCMD
}\DIFdelend 

\begin{align}
\label{eq:loglikelihood}
L \DIFaddbegin &\DIFaddend = \DIFdelbegin \DIFdel{-\log}%DIFDELCMD < \left|%%%
\DIFdel{S}%DIFDELCMD < \right| %%%
\DIFdel{- }\DIFdelend {\rm tr}\left(\DIFdelbegin \DIFdel{\Sigma }\DIFdelend S \DIFaddbegin \DIFadd{\Sigma}\DIFaddend ^{-1}\right) \DIFaddbegin \DIFadd{+ \log}\left|\DIFadd{\Sigma}\right|\DIFaddend ,
\end{align} \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend where \(|\cdot|\) denotes the determinant of a matrix and
\({\rm tr}(\cdot)\) denotes the trace \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{bollen2014structural}}\hskip0pt%DIFAUXCMD
}\DIFaddend . For
the perfect-mediation model, these values are

\begin{align}
\left|\DIFdelbegin \DIFdel{S}\DIFdelend \DIFaddbegin \DIFadd{\Sigma}\DIFaddend \right| &= (1-\alpha^2)(1-\beta^2) \\
%S^{-1} &= 
  %\begin{bmatrix}
...
%\end{bmatrix}\\ 
\DIFdelbegin \DIFdel{S}\DIFdelend \DIFaddbegin \DIFadd{\Sigma}\DIFaddend ^{-1} &= 
  \begin{bmatrix}
...
%DIFDELCMD < %%%
\DIFdelend Plugging these into the likelihood function, we get
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{align}
\label{eqn:mediation_loglikelihood}
L = &\DIFdelbegin \DIFdel{-}\DIFdelend \log\left((1-\alpha^2)(1-\beta^2)\right) \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD <     &%%%
\DIFdel{+ }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend \frac{2\alpha^2\beta^2}{(1-\alpha^2)(1-\beta^2)} \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD <     &%%%
\DIFdelend + \DIFaddbegin \DIFadd{1 - }\DIFaddend \frac{2\alpha}{1-\alpha^2} \DIFdelbegin \DIFdel{\Sigma}\DIFdelend \DIFaddbegin \DIFadd{S}\DIFaddend _{12} \DIFdelbegin \DIFdel{+ }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend \frac{2\beta}{1-\beta^2} \DIFdelbegin \DIFdel{\Sigma}\DIFdelend \DIFaddbegin \DIFadd{S}\DIFaddend _{23}
\DIFdelbegin \DIFdel{- 1
}\DIFdelend \end{align} \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend To simplify notation, we define \DIFdelbegin \[\DIFdel{F(\alpha,\beta) = -\log\left((1-\alpha^2)(1-\beta^2)\right) + \frac{2\alpha^2\beta^2}{(1-\alpha^2)(1-\beta^2)} + \frac{2\beta}{1-\beta^2} - 1,}\]%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{F(\alpha,\beta) = \log\left((1-\alpha^2)(1-\beta^2)\right) - \frac{2\alpha^2\beta^2}{(1-\alpha^2)(1-\beta^2)} + 1,
}\end{equation}\DIFaddend  so \DIFdelbegin \DIFdel{, }\DIFdelend the likelihood function is now \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{align}
\label{eqn:mediation_loglikelihood_simple}
L = F(\alpha, \beta) \DIFdelbegin \DIFdel{+ }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend \frac{2\alpha}{1-\alpha^2} \DIFdelbegin \DIFdel{\Sigma}\DIFdelend \DIFaddbegin \DIFadd{S}\DIFaddend _{12} \DIFdelbegin \DIFdel{+ }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend \frac{2\beta}{1-\beta^2} \DIFdelbegin \DIFdel{\Sigma}\DIFdelend \DIFaddbegin \DIFadd{S}\DIFaddend _{23}
\end{align} \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{Note that this }\DIFaddend likelihood is maximized by fitting regression
coefficients \(\alpha\) and \(\beta\) between \(x\) and \(m\) and \(m\)
and \(y\), respectively\DIFaddbegin \DIFadd{, but the negative log-likelihood formulation is
useful for the multivariate extension below}\DIFaddend .
...
\DIFdelbegin \begin{displaymath}
    {\DIFdel{\rm mean}}\DIFdel{(l_X) = }{\DIFdel{\rm mean}}\DIFdel{(l_M) = }{\DIFdel{\rm mean}}\DIFdel{(l_Y) = 0,
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
{\DIFadd{\rm mean}}\DIFadd{(l_X) = }{\DIFadd{\rm mean}}\DIFadd{(l_M) = }{\DIFadd{\rm mean}}\DIFadd{(l_Y) = 0,
...
\DIFdelbegin \begin{displaymath}
    {\DIFdel{\rm var}}\DIFdel{(l_X) = }{\DIFdel{\rm var}}\DIFdel{(l_M) = }{\DIFdel{\rm var}}\DIFdel{(l_Y) = 1.
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
{\DIFadd{\rm var}}\DIFadd{(l_X) = }{\DIFadd{\rm var}}\DIFadd{(l_M) = }{\DIFadd{\rm var}}\DIFadd{(l_Y) = 1.
}\end{equation}\DIFaddend 
...
\begin{align}
\DIFdelbegin \DIFdel{\Sigma}\DIFdelend \DIFaddbegin \DIFadd{S}\DIFaddend _{12} &= {\rm corr}\left(l_X, l_M\right)\\
\DIFdelbegin \DIFdel{\Sigma}\DIFdelend \DIFaddbegin \DIFadd{S}\DIFaddend _{23} &= {\rm corr}\left(l_M, l_Y\right)\\
\label{eqn:gcca_mediation}
L(\alpha, \beta, a,b,c) &= F(\alpha, \beta) \DIFdelbegin \DIFdel{+ }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend \frac{2\alpha}{1-\alpha^2} {\rm corr}\left(l_X, l_M\right) \DIFdelbegin \DIFdel{+ }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend \frac{2\beta}{1-\beta^2} {\rm corr}\left(l_M, l_Y\right) \\ 
&= F(\alpha, \beta) \DIFdelbegin \DIFdel{+ }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend \frac{2\alpha}{1-\alpha^2} {\rm corr}\left(Xa, Mb\right) \DIFdelbegin \DIFdel{+ }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend \frac{2\beta}{1-\beta^2} {\rm corr}\left(Mb, Yc\right)
\end{align}
...
converges at least to a stationary point. The overall idea is to use \DIFaddbegin \DIFadd{a
}\DIFaddend block-coordinate-ascent strategy that iterates between optimizing \(a\),
...
\(F(\alpha, \beta)\) is irrelevant, so \DIFdelbegin \DIFdel{our objective }\DIFdelend \DIFaddbegin \DIFadd{minimizing the negative
log-likelihood }\DIFaddend function reduces to \DIFaddbegin \DIFadd{maximizing the reduced function
}\DIFaddend 
...
\begin{align}
L_{wSUMCOR}(a,b,c, w_1, w_2) = w_1 {\rm corr}\left(Xa, Mb\right) + w_2 {\rm corr}\left(Mb, Yc\right)\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend \end{align}
\DIFaddbegin 

\DIFadd{Iterating between optimizing the structural parameters and loading
parameters, we reduce the negative log-likelihood at each step and
converge to a fixed point}\DIFaddend .
...
in the span of the \DIFaddbegin \DIFadd{the }\DIFaddend measured individuals, namely
...
among individuals \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(aka, Gram matrices), }\DIFaddend \(C_X = XX^T\), and reduces the
...
linear model to a \DIFaddbegin \DIFadd{non-linear model by replacing the covariance matrices,
}\textit{\DIFadd{e.g.}} \DIFadd{\(C_X\), with more complex }\textit{\DIFadd{kernel matrices}}
\DIFadd{\(K_X\) that encode similarity measures among individuals that are
non-linear functions of the measured variables. }\DIFaddend non-linear model by
