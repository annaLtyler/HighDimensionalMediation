...
and the total numer of islets per pancreas \mbox{%DIFAUXCMD
\cite{pmid29567659}}\hskip0pt%DIFAUXCMD
. RNA
was isolated from the whole islets and sent to The Jackson Laboratory
for high-throughput sequencing \mbox{%DIFAUXCMD
\cite{pmid29567659}}\hskip0pt%DIFAUXCMD
.
}

\DIFaddend ## Trait measurements
...
\cite{choi2020genotype} 
}\hskip0pt%DIFAUXCMD
}\DIFaddend (GBRS, https://github.com/churchill-lab/gbrs). \DIFaddbegin \DIFadd{Using both methods 
...
from the analysis \mbox{%DIFAUXCMD
\cite{pmid29567659}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 

## \DIFdelbegin \DIFdel{Pre-processed DO data
The DO data had been published previously 
\mbox{%DIFAUXCMD
\cite{pmid29567659, pmid31343992}}\hskip0pt%DIFAUXCMD
, We downloaded pre-processed }\DIFdelend \DIFaddbegin \DIFadd{Processed Data
We downloaded }\DIFaddend genotypes, phenotypes, and pancreatic islet gene expression 
data from Dryad (doi:10.5061/dryad.pj105).

## Collaborative cross recombinant inbred mice \DIFaddbegin \DIFadd{(CC-RIX)
}\DIFaddend Mice were cared for and treated following the guidelines approved by 
...
or submental vein. \DIFdelbegin \DIFdel{In a subset of the 12-month cohort, metformin 
(5 mg/mL) was administered }\textit{\DIFdel{ad libitum}} %DIFAUXCMD
...
of the experiment.
}\DIFdelend \DIFaddbegin \DIFadd{Sex, diet, and age were used as covariates in all
analyses.
}\DIFaddend 
...
and homogenized in TRIzol\DIFdelbegin \DIFdel{™ }\DIFdelend \DIFaddbegin \DIFadd{\textsuperscript{\texttrademark} 
}\DIFaddend Reagent (ThermoFisher; 15596026) using a 
...
\DIFdelbegin \DIFdel{~69K }\DIFdelend \DIFaddbegin \DIFadd{$\sim69$K }\DIFaddend markers, which was used for confirming genotypes in the quality control (QC) process.
...
8 for significant QTLs which corresponded to a \DIFaddbegin \DIFadd{nominal }\DIFaddend $p$ value of 0.05. 
...
distal eQTLs separately. \DIFaddbegin \DIFadd{Local eQTL were defined as an eQTL within
...
with the local genotype using the fit1() function in R/qtl\DIFdelbegin \DIFdel{with a kinship 
correction}\DIFdelend . We used the 
...
of trait relevance for each transcript. \DIFdelbegin \DIFdel{Trait relevance
was the Pearson correlation (R) between the transcript and the trait }\DIFdelend \DIFaddbegin \DIFadd{To calculate trait relevance
