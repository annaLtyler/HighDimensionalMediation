...
%DIF DEL High_Dimensional_Mediation_ms.tex            Wed Oct  9 11:27:48 2024
%DIF ADD High_Dimensional_Mediation_post_review.tex   Thu Feb 20 08:47:02 2025
...
increased interest in \DIFdelbegin \DIFdel{distal }\DIFdelend \DIFaddbegin \DIFadd{more distant (distal) genetic }\DIFaddend effects as potential
drivers of trait variation \cite{pmid37857933, 
pmid32424349, pmid32831138, pmid30950127, pmid24013639}. In \DIFdelbegin \DIFdel{particular,
the omnigenic
model }\DIFdelend \DIFaddbegin \DIFadd{general,
...
distal rather than }\textit{\DIFadd{cis}} \DIFadd{and }\textit{\DIFadd{trans}} \DIFadd{because }\textit{\DIFadd{cis}}
\DIFadd{and }\textit{\DIFadd{trans}} \DIFadd{have specific biochemical meanings
\mbox{%DIFAUXCMD
\cite{pmid18597885}}\hskip0pt%DIFAUXCMD
, whereas local and distal are defined only by
genomic position. The importance of distal genetic effects is proposed
in the omnigenic model, which }\DIFaddend posits that trait-driving genes are
...
We \DIFdelbegin \DIFdel{calculated }\DIFdelend \DIFaddbegin \DIFadd{estimated }\DIFaddend the heritability of each transcript in terms of local and
...
between 8 and \DIFdelbegin \DIFdel{18}\DIFdelend \DIFaddbegin \DIFadd{17}\DIFaddend \% of the variance in the median transcript (Fig,
\ref{fig:motivation}A).

\DIFaddbegin \DIFadd{The equal contribution of local and distal genetic variants to the
...
70-80\%\mbox{%DIFAUXCMD
\cite{pmid31558840, pmid21383966}}\hskip0pt%DIFAUXCMD
. This discrepancy may arise due
...
local eQTL\mbox{%DIFAUXCMD
\cite{pmid25010687}}\hskip0pt%DIFAUXCMD
. Because of the high diversity in the DO
...
\textbf{A.} Distributions of \DIFdelbeginFL \DIFdelFL{distal and }\DIFdelendFL local \DIFaddbeginFL \DIFaddFL{and distal }\DIFaddendFL heritability 
of transcripts across the four tissues. Overall local and 
distal factors \DIFdelbeginFL \DIFdelFL{contribute }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{contributed }\DIFaddendFL equally to transcript heritability. 
\DIFaddbeginFL \DIFaddFL{Arrows indicate the median of each distribution. }\DIFaddendFL The 
...
correlation between the transcript and all traits. \DIFdelbeginFL \DIFdelFL{Local heritability was negatively 
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The upper 
...
}\DIFaddendFL correlated with \DIFdelbeginFL \DIFdelFL{trait relevance, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{traits tended to have low local heritability }\DIFaddendFL and 
\DIFaddbeginFL \DIFaddFL{high }\DIFaddendFL distal heritability\DIFdelbeginFL \DIFdelFL{is 
positively correlated with trait relevance}\DIFdelendFL .\DIFdelbeginFL \DIFdelFL{Pearson ($r$) and $p$ 
values for each correlation are shown in the upper-right of each panel.}\DIFdelendFL }
...
transcripts to their trait relevance\DIFdelbegin \DIFdel{, defined as the maximum trait correlation for each transcript }\DIFdelend \DIFaddbegin \DIFadd{. We defined trait relevance for a
transcript as its maximum absolute Spearman correlation coefficient
(\(\rho\)) across all traits (Methods)}\DIFaddend . The local heritability of
transcripts was negatively \DIFdelbegin \DIFdel{correlated }\DIFdelend \DIFaddbegin \DIFadd{associated }\DIFaddend with their trait relevance (Fig.
...
was positively \DIFdelbegin \DIFdel{correlated }\DIFdelend \DIFaddbegin \DIFadd{associated }\DIFaddend with trait relevance (Fig.
...
\ref{fig:workflow}). \DIFdelbegin \DIFdel{HDMA projects the high-dimensional }\DIFdelend \DIFaddbegin \DIFadd{In HDMA, we first use a linear mapping called
kernelization to dimension-reduce the }\DIFaddend genome, transcriptome, and phenome
\DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{to kernel matrices \(G_K\), \(T_K\) and \(P_K\) respectively, which each
...
matrices }\DIFaddend onto one-dimensional scores--a composite genome score
(\(G_C\)), a composite transcriptome score (\(T_C\)), and a composite
phenome score (\(P_C\))--and \DIFdelbegin \DIFdel{uses }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend the univariate theory of mediation to
...
available in \DIFdelbegin \DIFdel{Supp. }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend Methods.
...
As discussed in \DIFdelbegin \DIFdel{Supp. }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend Methods, HDMA is related to a generalized form of
\DIFdelbegin \DIFdel{CCA}\DIFdelend \DIFaddbegin \DIFadd{canonical correlation analysis (CCA)}\DIFaddend . Standard CCA is prone to
...
\DIFaddbegin \DIFadd{permutations preserved the correlation between the genome and phenome,
...
apparently mediating transcriptomic signatures in random data. The }\DIFaddend null
...
suggesting that the path from genotype to phenotype through \DIFaddbegin \DIFadd{the
}\DIFaddend transcriptome is highly non-trivial and identifiable in this case. These
...
trait. \textbf{B.} Phenotype scores \DIFaddbeginFL \DIFaddFL{(MDI) }\DIFaddendFL across individuals. Individuals with 
large positive \DIFdelbeginFL \DIFdelFL{phenotype 
scores }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MDI }\DIFaddendFL had higher body weight and insulin 
resistance than average. Individuals with large negative \DIFdelbeginFL \DIFdelFL{phenotype scores }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MDI }\DIFaddendFL had 
...
scores. \textbf{D.} \DIFdelbeginFL \DIFdelFL{Distribution }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Distributions }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{absolute 
value of transcript }\DIFdelendFL loadings across tissues \DIFaddbeginFL \DIFaddFL{compared 
to null distributions}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{Shaded areas represent loadings that were more 
extreme than the null distribution. Numbers indicate how many transcripts 
had loadings above and below the extremes of the null. }\DIFaddendFL Transcripts in 
adipose tissue had the \DIFdelbeginFL \DIFdelFL{largest }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{most extreme }\DIFaddendFL loadings indicating that \DIFaddbeginFL \DIFaddFL{transcripts 
in }\DIFaddendFL adipose tissue \DIFdelbeginFL \DIFdelFL{gene expression was a strong mediator }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{were the best mediators }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{genotype }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the genetic effects }\DIFaddendFL on body 
...
Looking \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend the 10 most positively and negatively loaded transcripts from
...
mediated through gene expression in adipose tissue. \DIFaddbegin \DIFadd{This finding does
...
large portion of the genetic effect on obesity. }\DIFaddend The strongest loadings
...
plot). \DIFdelbegin \DIFdel{This pattern
contrasts with transcripts nominated by TWAS }\DIFdelend \DIFaddbegin \DIFadd{We also performed TWAS in this population by imputing transcript
...
procedure tended to nominate transcripts with lower loadings }\DIFaddend (Fig.
\ref{fig:loading_heritability}B), \DIFdelbegin \DIFdel{which tended to have lower loadings,
}\DIFdelend higher local heritability and lower
distal heritability. \DIFdelbegin \DIFdel{Transcripts
}\DIFdelend \DIFaddbegin \DIFadd{Finally, we focused on transcripts }\DIFaddend with the highest
local heritability in each tissue (Fig.
\ref{fig:loading_heritability}C)\DIFdelbegin \DIFdel{had the lowest loadings }\DIFdelend \DIFaddbegin \DIFadd{. This procedure selected transcripts
with low loadings on average}\DIFaddend , consistent with our findings above (Fig.
...
literature support. They were \DIFdelbegin \DIFdel{2.4 }\DIFdelend \DIFaddbegin \DIFadd{2.1 }\DIFaddend times more likely than TWAS hits and
...
and literature support \DIFaddbeginFL \DIFaddFL{(bolded gene names)}\DIFaddendFL . Each panel has a bar plot 
...
\DIFaddbeginFL \DIFaddFL{Distal heritability was significantly higher than local heritability 
(t-test $p < 2.2^{-16}$). }\DIFaddendFL \textbf{B.} Loadings of TWAS candidates 
with the 10 largest positive correlations with traits and the largest 
negative correlations with traits across all four tissues. \DIFaddbeginFL \DIFaddFL{Local and 
distal heritability were not significantly different for this group 
(t-test $p = 0.77$). }\DIFaddendFL \textbf{C.} The transcripts with the largest 
local heritability (top 20) across all four tissues. \DIFaddbeginFL \DIFaddFL{Local heritability 
...
of heritable transcript variability in driving phenotype. \DIFdelbegin %DIFDELCMD < 
...
(\DIFaddbegin \DIFadd{Holm-adjusted two-sided empirical \(p < 2\times10^{-16}\)) (}\DIFaddend Fig.
...
\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{(Adipocytes: \(p = 0.08\), Progenitors: \(p = 0.58\), Leukocytes:
\(p = 0.28\); all Holm-adjusted two-sided empirical p), }\DIFaddend indicating that
changes in the abundance of those cell types \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend not a mediator of MDI.
...
significantly greater than 0 \DIFaddbegin \DIFadd{(\(p = 0.002\))}\DIFaddend , while the mean loadings
for delta- \DIFaddbegin \DIFadd{(Holm-adjusted two-sided empirial \(p < 2\times10^{-16}\))
}\DIFaddend and endothelial-cell \DIFaddbegin \DIFadd{(Holm-adjusted two-sided empirical \(p = 0.01\))
}\DIFaddend specific genes were significantly less than 0 (Fig.
...
\DIFaddbegin \DIFadd{(Holm-adjusted two-sided empirical \(p = 0.95\))}\DIFaddend . We stress that this is
...
(CMAP) database \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{pmid17008526} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{pmid17008526, 
pmid29195078} }\hskip0pt%DIFAUXCMD
...
part, by inhibiting mTOR signaling \mbox{%DIFAUXCMD
\cite{pmid30290005, 
pmid30034573}}\hskip0pt%DIFAUXCMD
. However, long-term use of other mTOR inhibitors, such as
rapamycin, are known to cause insulin resistance and \(\beta\)-cell
toxicity \mbox{%DIFAUXCMD
\cite{pmid30034573, pmid23881200, pmid21266327}}\hskip0pt%DIFAUXCMD
.
...
hyperglycemia and diabetes \mbox{%DIFAUXCMD
\cite{pmid24582093, pmid35585199}}\hskip0pt%DIFAUXCMD
. Accute
treatment with glucocorticoids has further been shown to reduce
thermogenesis in rodent adipocytes \mbox{%DIFAUXCMD
\cite{pmid30310815, 
pmid11254472, pmid23197361}}\hskip0pt%DIFAUXCMD
, but increase thermogenesis in human
adipocytes \mbox{%DIFAUXCMD
\cite{pmid27411014, pmid25385872}}\hskip0pt%DIFAUXCMD
. Thus, the pathways
...
%DIFDELCMD < %%%
\DIFdel{The top hit for the adipose composite transcript in CMAP adipocytes was
...
and diabetes \mbox{%DIFAUXCMD
\cite{pmid34450194}}\hskip0pt%DIFAUXCMD
. PARP1 inhibition increases
mitochondrial biogenesis \mbox{%DIFAUXCMD
\cite{pmid21459330}}\hskip0pt%DIFAUXCMD
. Inihibition of PARP1
activity can further prevent necrosis in favor of the less inflammatory
apoptosis \mbox{%DIFAUXCMD
\cite{pmid12114611}}\hskip0pt%DIFAUXCMD
, thereby potentially reducing inflammation
...
improve insulin resistance \mbox{%DIFAUXCMD
\cite{pmid33648925} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{The CMAP database identified both known diabetes drugs
(e.g.~sulfonylureas), }\DIFaddend as well as \DIFdelbegin \DIFdel{to reduce
insulin antibodies in type I diabetes \mbox{%DIFAUXCMD
\cite{pmid28753229}}\hskip0pt%DIFAUXCMD
. IkappaB
kinase (IKK) is an enzyme complex involved in regulating cellular
responses to inflammation \mbox{%DIFAUXCMD
\cite{pmid17047224}}\hskip0pt%DIFAUXCMD
. Inhibitors of IKK have
been shown to improve glucose control in type II diabetes
\mbox{%DIFAUXCMD
\cite{pmid28683283, 
pmid15685170}}\hskip0pt%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Among the top most significant hits for the transcript loadings from
pancreatic islets (Supp. Fig. \ref{fig:clue_islet_all}), was suppression
of T cell receptor signaling, which is }\DIFdelend \DIFaddbegin \DIFadd{drugs that target pathways }\DIFaddend known to be
involved in \DIFdelbegin \DIFdel{Type 1
diabetes \mbox{%DIFAUXCMD
\cite{pmid33603744}}\hskip0pt%DIFAUXCMD
, as well as TNFR1, which has been
associated with mortality in diabetes patients \mbox{%DIFAUXCMD
\cite{pmid32281000}}\hskip0pt%DIFAUXCMD
. Suppression of NOD1/2 signaling was also among the top hits. NOD1 and 2
sense ER stress \mbox{%DIFAUXCMD
\cite{pmid27007849, pmid28823510}}\hskip0pt%DIFAUXCMD
, which is associated
with \(\beta\)-cell death in type 1 and type 2 diabetes
\mbox{%DIFAUXCMD
\cite{pmid24520198}}\hskip0pt%DIFAUXCMD
. This cell death process is dependent on NOD1/2
signaling \mbox{%DIFAUXCMD
\cite{pmid27007849}}\hskip0pt%DIFAUXCMD
, although the specifics have not
yet been
worked out. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We also looked specifically at hits in pancreatic tumor cells (YAPC)
...
PPAR receptor agonists, and insulin sensitizers. Rosiglitazone is a PPAR-\(\gamma\) agonist and was one of the most prescribed drugs for
type 2 diabetes before its use was reduced due to cardiac side-effects
\mbox{%DIFAUXCMD
\cite{pmid21190462}}\hskip0pt%DIFAUXCMD
. Sulfonylureas are another commonly prescribed drug
class for type 2 diabetes, but also have notable side effects including
hypoglycemia and accellerated \(\beta\)-cell death \mbox{%DIFAUXCMD
\cite{pmid16631807}}\hskip0pt%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In summary, the high-loading transcripts derived from HDMA in
mice
...
incidentally to reduce obesity-related morbidity}\DIFdelend \DIFaddbegin \DIFadd{diabetes pathogenesis (e.g.~mTOR inhibitors). These findings
...
\DIFdelbegin \subsection{\DIFdel{Supplemental Discussion}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{supplemental-discussion}
%DIFDELCMD < 
...
the \DIFdelbegin \DIFdel{causal role of these full gene signatures }\DIFdelend \DIFaddbegin \DIFadd{hypothesis that HDMA identified transcripts that truly mediate
...
model and the hypothesis that these transcripts have a causal role }\DIFaddend in
pathogenesis of metabolic disease\DIFdelbegin \DIFdel{and thus their utility in prioritizing drugs and gene
targets as therapeutics}\DIFdelend .
...
\DIFdelbegin %DIFDELCMD < \url{https://figshare.com/}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \url{https://figshare.com/articles/dataset/Data_and_code_for_High-Dimensional_Mediation_Analysis_HDMA_in_diversity_outbred_mice/27066979}
\DIFaddend DOI: 10.6084/m9.figshare.27066979

\DIFaddbegin \DIFadd{10.6084/m9.figshare.27066979.v1
}

\DIFaddend \textbf{CC-RIX mice}: Gene expression can be found at the Gene
...
\begin{figure}[ht!]
\DIFaddbeginFL \includegraphics[width=\textwidth]{Figures/Supp_Fig_Composite_Vector_Correlations.pdf} 
\caption{\DIFaddFL{Scatter plots showing correlations between composite vectors 
...
\begin{figure}[ht!]
\DIFaddendFL \includegraphics[width=\textwidth]{Figures/Supp_Fig_enrichments_KEGG.pdf} 
...
that increased expression of genes involved in \DIFdelbeginFL \DIFdelFL{branched-chain amino acid 
degradation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the TCA cycle }\DIFaddendFL was 
...
All distributions were skewed toward positive correlations and had
 ...
 local eQTLs. \textbf{C.} \DIFdelbeginFL \DIFdelFL{Variance }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Distributions of variance }\DIFaddendFL explained by local 
 genotype \DIFaddbeginFL \DIFaddFL{across all transcripts }\DIFaddendFL in the DO and CC-RIX. 
}
