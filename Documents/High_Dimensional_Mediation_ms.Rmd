...
pmid32831138, pmid30950127}. In this model, a gene's expression 
is influenced by many variants throughout the genome through their 
cumulative effects on a broader regulatory network. In other 
...
the DO cohort (Supp. Fig. \ref{fig:eQTL}A-H). We performed eQTL 
...
technique, high-dimension mediation analytis (HDMA), that uses the 
...
(Fig. \ref{fig:workflow}). HDMA projects the high-dimensional scores--a 
composite genome score ($G_C$), a composite transcriptome score ($T_C$), 
...
score, the genome score is uncorrelated to the phenome score. Formally, 
...
positively associated with metabolic index, indicating that 
increased expression in inflammatory pathways was associated 
with a higher metabolic index. It is well established that 
...
on a HFHS diet \cite{pmid18492779}.
...
the HFHS diet. Loading for marker genes for other cell 
